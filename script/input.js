...
function printChecks(id_etiqueta,array_categorias) {
    ...
    array_categorias = array_categorias.map(each=> {
        ...
        <div class="form-check form-check-inline">
            <input onclick="captureData()" class="class_checks form-check-label" type="checkbox" value="${each}" name="tipo" id="${each}">
    ...
    <input onkeyup="captureData()" class="form-control me-2" type="text" placeholder="Search" aria-label="Search" id="searchInput" name="texto">
    </form>
</div>`)
